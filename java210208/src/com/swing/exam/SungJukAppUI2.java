...
public class SungJukAppUI2 extends JFrame implements ActionListener {
	...
	JButton jbtn_inputData = new JButton("데이터추가");
...
/*			dtm.setValueAt(aValue, row, column);
		...
		else if(e.getSource()==jbtn_inputData) {
			try {
				if(dtm.getClass()==null) {
				}
			} catch (NullPointerException e2) {
				JOptionPane.showMessageDialog(this, "표를 먼저 만들어주세요");
				return;
				}
			String[][] data = {
					 {"홍길동","80","75","85"}
			...
			for(int i=0;i<3;i++){
				...
				for(int j=0;j<4;j++){
					...
					JOptionPane.showMessageDialog(this, "3명 이상 테이블을 만들어주세요.");
					return;
				}
