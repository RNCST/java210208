...
public class BaseBallGameView {
	//컴파일 타임에 인스턴스화 하기 - 이른 인스턴스화이다.(멀티스레드에서 안전하지 못함)
	BaseBallGameLogic gl = new BaseBallGameLogic(); 
	BaseBallGameEvent ge = new BaseBallGameEvent(this, gl);
	...
	public void initDisplay(){
		...
		for(int i=0;i<4;i++){
			...
			jp_east.add(jbtns[i]);
			jbtns[i].addActionListener(ge);
			
		}
		//이벤트소스와 이벤트핸들러 클래스 매핑 코드
		jtf_input.addActionListener(ge);
